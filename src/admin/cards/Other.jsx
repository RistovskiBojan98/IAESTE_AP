...
const Other = ({ selectedCountry }) => {
    ...
    useEffect(() => {
        if (selectedCountry) {
            const result = otherInfo.find((obj) => obj.country === selectedCountry?.name)?.info ?? [];
            setOtherData(result)
        }
    }, [selectedCountry]);
    ...
    const handleInputDescChange = (e, index) => {
        const newData = [...otherData];
        newData[index].desc = e.target.value; // Update the number in the contact data
        setOtherData(newData);
    };
    ...
    const handleDeleteClick = (index) => deleteCardItem(index, otherData, setOtherData)

    // Handle save new item
    const handleSaveNewItem = () =>  {
        const newData = [...otherData]
        const newItem = { title: inputTitleValue, desc: inputValue }
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
            {!!otherData.length && otherData.map((item, index) => (
            ...
            <div>
                {!isAddMode ? (
                    ...
                    >
                        ...
                        <span className="font-semibold">Add a new useful information</span>
                    </button>
                ) : (
